...
覆盖：IP Helper/Toolhelp 进程信息、cmd 引号、PPID 树（含环）、PEB cwd、
...
class WindowsParsingTests(unittest.TestCase):
    ...
    def test_native_process_table_contains_current_process(self):
        info = server._win_process_table().get(server.SELF_PID)
        self.assertIsNotNone(info)
        self.assertTrue(info.get("name"))
        self.assertIn("python", (info.get("args") or "").casefold())
        self.assertIsInstance(info.get("ppid"), int)
        self.assertTrue(info.get("sid"))
        self.assertEqual(server.ps_snapshot({server.SELF_PID})[server.SELF_PID]["uid"], 0)
...
class WindowsProcessTests(unittest.TestCase):
    def test_external_process_controls_are_disabled(self):
        ok, error = server.kill_process(server.SELF_PID, False)
        self.assertFalse(ok)
        self.assertIn("不允许结束外部进程", error)
        ok, error, info = server.inspect_attach_process(
            mock.Mock(), {"kind": "service", "port": 8000}, server.SELF_PID)
        self.assertFalse(ok)
        self.assertEqual(info["status"], 403)
        self.assertIn("不认领", error)
    ...
    def test_service_lifecycle_managed_stop(self):
        ...
        with tempfile.TemporaryDirectory() as td, \
            ...
            try:
                ...
                deadline = time.time() + 10
                listening_ports = set()
                while time.time() < deadline and port not in listening_ports:
                    time.sleep(0.1)
                    listening_ports = {p for _, p in server.scan_listeners()}
                self.assertIn(port, listening_ports)
                stopped, error = server.stop_app_and_clear(
                    cfg, tracked, timeout=10)
                self.assertTrue(stopped, error)
                proc.wait(timeout=3)
                self.assertTrue(
                    server.wait_app_exit_watcher(base["id"], token, timeout=1),
                    "退出监视线程未完成配置/日志收尾")
                self.assertFalse(server.pid_alive(proc.pid))
