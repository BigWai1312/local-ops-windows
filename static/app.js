'use strict';
/* ============================================================
   app.js — 入口：视图切换 / 轮询 / 命令面板 / 本地运维台 Windows 自身
   ============================================================ */
  ...
  const run = (async () => {
    try {
      ...
      if (!document.hidden || timedOut) {
        ...
        setConnected(false, denied ? '控制台拒绝了当前页面的访问，请重新打开本地运维台 Windows。' : '');
      }
    } finally {
...
function render() {
  ...
  restartConsoleBtn.setAttribute('aria-label', restartSupported ? '重启本地运维台 Windows' : '启用一键重启');
  restartConsoleBtn.title = restartSupported
    ? '重启本地运维台 Windows · PID ' + consolePid +
...
function showConsoleActivationInfo(action) {
  openConfirm({
    title: '先启用后台控制',
    bodyHtml: '当前 <b>' + escapeHtml(consolePortLabel.textContent || '本地运维台 Windows') +
...
restartConsoleBtn.addEventListener('click', () => {
  ...
  openConfirm({
    title: '重启本地运维台 Windows',
    bodyHtml: '确定要重启本地运维台 Windows 吗？' +
    ...
    onOk: async () => {
      ...
      banner.textContent = '本地运维台 Windows 正在重新启动，页面会自动恢复…';
      banner.classList.add('show');
      banner.setAttribute('aria-hidden', 'false');
      render();
      ...
      restartDeadlineTimer = setTimeout(() => {
        if (!state.restartingFrom) return;
        state.restartingFrom = null;
        setConnected(false, '本地运维台 Windows 重启超时，请双击“' + launcherName() + '”重新打开。');
        render();
      }, 25000);
    },
  });
...
stopConsoleBtn.addEventListener('click', () => {
  ...
  openConfirm({
    title: '停止本地运维台 Windows',
    bodyHtml: '确定要停止本地运维台 Windows 吗？' +
      '<div class="confirm-detail">当前页面会断开；启动台里已经运行的应用不会被停止。再次使用时，双击“' + launcherName() + '”即可。</div>',
    okText: '停止运行',
    onOk: async () => {
      state.stopping = true;
      banner.textContent = '本地运维台 Windows 正在停止…再次启动请双击“' + launcherName() + '”。';
      banner.classList.add('show');
      banner.setAttribute('aria-hidden', 'false');
      render();
      ...
      banner.textContent = '本地运维台 Windows 已停止。再次启动请双击“' + launcherName() + '”。';
    },
  });
});
...
/* Ctrl+J / Ctrl+J 呼出日志中心（Ctrl+L 是浏览器地址栏保留键，无法拦截） */
document.addEventListener('keydown', e => {
